        ...
        <div>
            <div class="d-flex mb-3 p-3 bg-light justify-content-between align-items-center">
                <div class="d-flex align-items-center">
                    <!-- Display user's profile picture or default if not set -->
                    <img src="uploads/<?= isset($user['p_p']) ? $user['p_p'] : 'default.jpg' ?>" class="w-25 rounded-circle">
                    <h3 class="fs-xs m-2"><?= isset($user['name']) ? $user['name'] : 'User' ?></h3> 
                </div>
                ...
                <?php if (!empty($conversations)) { ?>
                    ...
                    <li class="list-group-item">
                        <a href="chat.php?user=<?= $conversation['username'] ?>" class="d-flex justify-content-between align-items-center p-2">
                            <div class="d-flex align-items-center">
                                <img src="uploads/<?= isset($conversation['p_p']) ? $conversation['p_p'] : 'default.jpg' ?>" class="w-10 rounded-circle">
                                <h3 class="fs-xs m-2">
                                    <?= isset($conversation['name']) ? $conversation['name'] : 'No Name' ?><br>
                                    <small>
                                        <?php 
...
<?php
    ...
    // Redirect to login if user is not logged in or session variables are not set
